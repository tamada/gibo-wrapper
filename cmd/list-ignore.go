...
func init() {
	wrapperCmd.AddCommand(listIgnoreCmd)
}
...
func findRegisteredBoilerplates(args []string) ([]string, error) {
	...
	fmt.Printf("filepath.Base(%s): %s (%v)\n", gitIgnorePath, filepath.Base(gitIgnorePath), isDirectory(gitIgnorePath))
...
func extractBoilerplateName(line string) string {
	...
	return strings.TrimSuffix(line[index+1:], gitignoreFileName)
}

func isBoilerplateName(line string) bool {
	return strings.HasPrefix(line, "###") && strings.HasSuffix(line, gitignoreFileName)
}
